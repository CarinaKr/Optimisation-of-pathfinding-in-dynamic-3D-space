//this class is part of the NativeHeap repository on GitHub by the user Amarcolina
    ...
    public struct NativeHeap<T, U> : IDisposable
        ...
        public bool TryPop(out T t) {
            unsafe {
                ...
                HeapNode rootNode = ReadArrayElement<HeapNode>(_data->Heap, 0);
        ...
        public void UpdateNode(NativeHeapIndex index, in T t)
        {
            unsafe
            {
#if NHEAP_SAFE
...
#endif
                int indexToUpdate = _data->Table[index.TableIndex].HeapIndex;

                HeapNode toUpdate = ReadArrayElement<HeapNode>(_data->Heap, indexToUpdate);
                toUpdate.Item = t;
                
                if (indexToUpdate != 0) 
                {
                    int parentIndex = (indexToUpdate - 1) / 2;
                    var parentNode = ReadArrayElement<HeapNode>(_data->Heap, parentIndex);
                    if (_comparator.Compare(toUpdate.Item, parentNode.Item) < 0) 
                    {
                        InsertAndBubbleUp(toUpdate, indexToUpdate);
                        return;
                    }
                }

                //If we couldn't bubble up, bubbling down instead
                InsertAndBubbleDown(toUpdate, indexToUpdate);
            }
        ...
        private void InsertAndBubbleDown(HeapNode node, int insertIndex) {
            unsafe
            {
                ...
                for (int indexL = insertIndex * 2 + 1; indexL < dataCount; indexL = insertIndex * 2 + 1)
                {
                    int indexR = indexL + 1;

                    if (indexR >= dataCount || _comparator.Compare(ReadArrayElement<HeapNode>(_data->Heap, indexL).Item, ReadArrayElement<HeapNode>(_data->Heap, indexR).Item) <= 0) 
                    {
